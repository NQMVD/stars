...
public class HomeView extends ScrollPane implements Searchable {
    ...
    public HomeView(RootLayout rootLayout) {
        ...
        featuredHeader.getChildren().addAll(new FontIcon(Feather.STAR), featuredTitle);
        ...
        trendingHeader.getChildren().addAll(new FontIcon(Feather.TRENDING_UP), trendingTitle);
        ...
        allAppsHeader.getChildren().addAll(new FontIcon(Feather.GRID), allAppsTitle);
        ...
        content.getChildren().addAll(featuredSection, trendingSection, allAppsSection);
        setContent(content);
    ...
    private void loadFeaturedApps() {
        ApiService.getInstance().getFeaturedApps().thenAccept(apps -> {
            Platform.runLater(() -> {
                ...
                if (apps.isEmpty()) {
                    ...
                    for (int i = 0; i < Math.min(3, apps.size()); i++) {
                        ...
                        HeroCard card = new HeroCard(
                            app.getName(), 
                            app.getDescription() != null ? app.getDescription() : "No description",
                            highlight
                        );
    ...
    private void loadTrendingApps() {
        ApiService.getInstance().getApps().thenAccept(apps -> {
            Platform.runLater(() -> {
                ...
                if (apps.isEmpty()) {
                    Label noApps = new Label("No apps available. Is the backend running?");
                    noApps.setStyle("-fx-text-fill: #71717a;");
