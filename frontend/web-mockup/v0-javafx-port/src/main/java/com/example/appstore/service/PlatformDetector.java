...
public class PlatformDetector {
     ...
     */
    public static boolean isAssetForPlatform(String assetName, Platform platform) {
        String name = assetName.toLowerCase();
        String arch = getArchitecture();
        
        // Filter out source archives
        if (name.endsWith(".tar.gz") && (name.contains("source") || name.contains("src"))) {
            return false;
        }
        ...
        switch (platform) {
            ...
            case MACOS:
                return (name.endsWith(".dmg") || name.endsWith(".pkg") || name.endsWith(".zip") || name.endsWith(".app.tar.gz"))
            ...
            case LINUX_DEB:
                ...
                return name.endsWith(".appimage") && isArchCompatible(name, arch);
            ...
            case LINUX_GENERIC:
                ...
                return name.endsWith(".tar.gz") && (name.contains("linux") || name.contains("nux"))
                       && !name.contains("source") && isArchCompatible(name, arch);
    ...
    private static boolean isArchCompatible(String name, String arch) {
        ...
        boolean hasArchInName = name.contains("x64") || name.contains("x86_64") || 
                                name.contains("amd64") || name.contains("arm64") ||
                                name.contains("aarch64") || name.contains("x86") ||
                                name.contains("i386") || name.contains("i686") ||
        ...
        if ("x64".equals(arch)) {
            return name.contains("x64") || name.contains("x86_64") || 
                   name.contains("amd64") || name.contains("universal");
        } else if ("arm64".equals(arch)) {
            return name.contains("arm64") || name.contains("aarch64") || 
                   name.contains("universal");
        } else if ("x86".equals(arch)) {
            return name.contains("x86") || name.contains("i386") || 
                   name.contains("i686") || name.contains("win32");
        }
    ...
    public static int getAssetPriority(String assetName, Platform platform) {
        ...
        switch (platform) {
            case WINDOWS:
                ...
                if (name.endsWith(".exe") && !name.contains("portable")) return 8;
                if (name.endsWith(".exe") && name.contains("portable")) return 5;
            ...
            case LINUX_ARCH:
                if (name.endsWith(".pkg.tar.zst") || name.endsWith(".pkg.tar.xz")) return 10;
            ...
            case LINUX_GENERIC:
                ...
                if (name.endsWith(".tar.gz") && name.contains("linux")) return 5;
                break;
        }
    ...
    public static String getInstallDirectory(Platform platform) {
        ...
        switch (platform) {
            case WINDOWS:
                ...
                return appData != null ? appData + "\\Stars\\Apps" : userHome + "\\AppData\\Local\\Stars\\Apps";
    ...
    public static String getPlatformDisplayName(Platform platform) {
        switch (platform) {
            ...
            case LINUX_DEB: return "Linux (Debian/Ubuntu)";
            case LINUX_RPM: return "Linux (Fedora/RHEL)";
            case LINUX_ARCH: return "Linux (Arch)";
