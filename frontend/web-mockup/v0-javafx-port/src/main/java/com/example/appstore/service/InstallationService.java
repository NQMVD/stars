...
public class InstallationService {
    ...
    public static class InstallProgress {
        ...
        public boolean isCompleted() { return stage == Stage.COMPLETED; }
    }
    ...
    public static class InstallResult {
        ...
        public InstallResult(String appId, String version, String installPath, 
                            String executablePath, long sizeBytes) {
            this.appId = appId;
     ...
     */
    public CompletableFuture<InstallResult> installApp(App app, Consumer<InstallProgress> progressCallback) {
        LOG.info("[InstallationService] Starting installation for: " + app.getName() + " (id: " + app.getId() + ")");
        return CompletableFuture.supplyAsync(() -> {
            try {
                return doInstall(app, progressCallback);
            } catch (Exception e) {
                LOG.log(Level.SEVERE, "[InstallationService] Installation failed for: " + app.getName(), e);
    ...
    private InstallResult doInstall(App app, Consumer<InstallProgress> progressCallback) throws Exception {
        Platform platform = PlatformDetector.detectPlatform();
        LOG.info("[InstallationService] Detected platform: " + PlatformDetector.getPlatformDisplayName(platform) + 
                 ", arch: " + PlatformDetector.getArchitecture());
        
        // Stage 1: Fetch release info
        LOG.info("[InstallationService] Stage 1: Fetching release info for " + app.getId());
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.FETCHING_RELEASE, 0.0, "Fetching release information..."
        ));
        ...
        if (release == null) {
            LOG.warning("[InstallationService] No release found for: " + app.getName());
            throw new RuntimeException("No release found for " + app.getName());
        }
        
        LOG.info("[InstallationService] Found release: " + release.getTagName() + " (" + release.getName() + ")");
        
        List<GithubAsset> assets = release.getAssets();
        if (assets == null || assets.isEmpty()) {
            LOG.warning("[InstallationService] No assets in release for: " + app.getName());
            throw new RuntimeException("No downloadable assets found for " + app.getName());
        }
        
        LOG.info("[InstallationService] Release has " + assets.size() + " assets:");
        for (GithubAsset asset : assets) {
            LOG.fine("[InstallationService]   - " + asset.getName() + " (" + formatBytes(asset.getSize()) + ")");
        }
        
        // Find the best asset for this platform
        GithubAsset bestAsset = findBestAsset(assets, platform);
        if (bestAsset == null) {
            LOG.warning("[InstallationService] No compatible asset found for platform: " + platform);
            throw new RuntimeException("No compatible download found for " + 
                PlatformDetector.getPlatformDisplayName(platform));
        }
        
        LOG.info("[InstallationService] Selected asset: " + bestAsset.getName() + 
                 " (" + formatBytes(bestAsset.getSize()) + ")");
        
        // Stage 2: Download
        LOG.info("[InstallationService] Stage 2: Downloading " + bestAsset.getName());
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.DOWNLOADING, 0.0, "Downloading " + bestAsset.getName() + "..."
        ));
        ...
        LOG.info("[InstallationService] Download completed in " + downloadTime + "ms, saved to: " + downloadPath);
        
        // Stage 3: Extract/Install
        LOG.info("[InstallationService] Stage 3: Installing from " + bestAsset.getName());
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.EXTRACTING, 0.0, "Extracting..."
        ));
        
        long installStart = System.currentTimeMillis();
        Path installPath = installAsset(app, downloadPath, bestAsset.getName(), platform, progressCallback);
        long installTime = System.currentTimeMillis() - installStart;
        LOG.info("[InstallationService] Installation completed in " + installTime + "ms, installed to: " + installPath);
        
        // Stage 4: Verify
        LOG.info("[InstallationService] Stage 4: Verifying installation");
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.VERIFYING, 0.0, "Verifying installation..."
        ));
        
        String executablePath = findExecutable(installPath, app.getName(), platform);
        LOG.info("[InstallationService] Found executable: " + executablePath);
        
        // Cleanup download
        try {
            Files.deleteIfExists(downloadPath);
            LOG.fine("[InstallationService] Cleaned up download file: " + downloadPath);
        } catch (IOException e) {
            LOG.warning("[InstallationService] Failed to cleanup download: " + e.getMessage());
        }
        
        // Stage 5: Complete
        LOG.info("[InstallationService] Stage 5: Installation complete for " + app.getName());
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.COMPLETED, 1.0, "Installation complete!"
        ));
     ...
     */
    private GithubAsset findBestAsset(List<GithubAsset> assets, Platform platform) {
        return assets.stream()
            .filter(asset -> PlatformDetector.isAssetForPlatform(asset.getName(), platform))
            .max(Comparator.comparingInt(asset -> 
     ...
     */
    private Path downloadAsset(GithubAsset asset, Consumer<Double> progressCallback) throws IOException {
     ...
     */
    private Path installAsset(App app, Path downloadPath, String assetName, Platform platform,
                              Consumer<InstallProgress> progressCallback) throws Exception {
        String fileName = assetName.toLowerCase();
        String appName = sanitizeAppName(app.getName());
        
        switch (platform) {
            case MACOS:
                return installMacOS(downloadPath, fileName, appName, progressCallback);
            case WINDOWS:
                return installWindows(downloadPath, fileName, appName, progressCallback);
            case LINUX_DEB:
            case LINUX_RPM:
            case LINUX_ARCH:
            case LINUX_GENERIC:
                return installLinux(downloadPath, fileName, appName, platform, progressCallback);
            default:
     ...
     * macOS installation.
     */
    private Path installMacOS(Path downloadPath, String fileName, String appName,
                              Consumer<InstallProgress> progressCallback) throws Exception {
     ...
     */
    private Path installDMG(Path dmgPath, String appName, 
                            Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.2, "Mounting disk image..."
        ));
        ...
        try {
            reportProgress(progressCallback, new InstallProgress(
                InstallProgress.Stage.INSTALLING, 0.5, "Copying application..."
            ));
     ...
     */
    private Path installPKG(Path pkgPath, Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.3, "Running installer..."
        ));
     ...
     */
    private Path installWindows(Path downloadPath, String fileName, String appName,
                                Consumer<InstallProgress> progressCallback) throws Exception {
        String installDir = PlatformDetector.getInstallDirectory(Platform.WINDOWS);
        ...
        if (fileName.endsWith(".msi")) {
            ...
            if (fileName.contains("portable") || fileName.contains("standalone")) {
                Path target = appDir.resolve(downloadPath.getFileName());
                Files.copy(downloadPath, target, StandardCopyOption.REPLACE_EXISTING);
                return target;
            } else {
        ...
        throw new RuntimeException("Unsupported Windows installer: " + fileName);
    }

    /**
     * Install MSI on Windows.
     */
    private Path installMSI(Path msiPath, Path targetDir, 
                            Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.3, "Running Windows Installer..."
        ));
        
        ProcessBuilder pb = new ProcessBuilder(
            "msiexec", "/i", msiPath.toString(), 
            "/quiet", "/norestart",
     ...
     */
    private Path runExeInstaller(Path exePath, Path targetDir,
                                  Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.3, "Running installer..."
        ));
        ...
        ProcessBuilder pb = new ProcessBuilder(
            exePath.toString(), "/S", "/D=" + targetDir.toString()
        );
        pb.redirectErrorStream(true);
        Process process = pb.start();
     ...
     * Linux installation.
     */
    private Path installLinux(Path downloadPath, String fileName, String appName,
                              Platform platform, Consumer<InstallProgress> progressCallback) throws Exception {
     ...
     */
    private Path installAppImage(Path appImagePath, String appName) throws Exception {
        String installDir = PlatformDetector.getInstallDirectory(Platform.LINUX_GENERIC);
        Files.createDirectories(Paths.get(installDir));
        
        Path targetPath = Paths.get(installDir, appName + ".AppImage");
        Files.copy(appImagePath, targetPath, StandardCopyOption.REPLACE_EXISTING);
     ...
     */
    private Path installDeb(Path debPath, Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.3, "Installing package..."
        ));
        
        // Try dpkg first (may require sudo)
        ProcessBuilder pb = new ProcessBuilder(
            "pkexec", "dpkg", "-i", debPath.toString()
        );
        pb.redirectErrorStream(true);
        Process process = pb.start();
     ...
     */
    private Path installRpm(Path rpmPath, Consumer<InstallProgress> progressCallback) throws Exception {
        reportProgress(progressCallback, new InstallProgress(
            InstallProgress.Stage.INSTALLING, 0.3, "Installing package..."
        ));
        
        ProcessBuilder pb = new ProcessBuilder(
            "pkexec", "rpm", "-i", rpmPath.toString()
        );
        pb.redirectErrorStream(true);
        Process process = pb.start();
        ...
        if (exitCode != 0) {
            ...
            pb = new ProcessBuilder(
                "pkexec", "dnf", "install", "-y", rpmPath.toString()
            );
            pb.start().waitFor();
        }
     ...
     * Extract a ZIP file.
     */
    private Path extractZip(Path zipPath, Path targetDir, String appName) throws IOException {
     ...
     */
    private Path extractTarGz(Path tarGzPath, Path targetDir, String appName) throws Exception {
        Path appDir = targetDir.resolve(appName);
        Files.createDirectories(appDir);
        
        ProcessBuilder pb = new ProcessBuilder(
            "tar", "-xzf", tarGzPath.toString(), "-C", appDir.toString()
        );
        pb.redirectErrorStream(true);
        Process process = pb.start();
     ...
     */
    private String findExecutable(Path installPath, String appName, Platform platform) {
    ...
    public void launchApp(String executablePath) throws IOException {
        Platform platform = PlatformDetector.detectPlatform();
        LOG.info("[InstallationService] Launching app: " + executablePath + " on platform: " + platform);
        
        ProcessBuilder pb;
        if (platform == Platform.MACOS) {
            if (executablePath.endsWith(".app")) {
                LOG.fine("[InstallationService] Using 'open' command for .app bundle");
        ...
        pb.redirectErrorStream(true);
        Process process = pb.start();
        LOG.info("[InstallationService] App launched successfully, PID: " + process.pid());
    }
    ...
    public boolean uninstallApp(String installPath) {
        ...
        try {
            ...
            if (Files.exists(path)) {
                ...
                LOG.info("[InstallationService] Successfully uninstalled: " + installPath);
                return true;
            } else {
                LOG.warning("[InstallationService] Install path does not exist: " + installPath);
            }
        } catch (IOException e) {
            LOG.log(Level.SEVERE, "[InstallationService] Failed to uninstall: " + installPath, e);
        }
        return false;
    }
    ...
    private void reportProgress(Consumer<InstallProgress> callback, InstallProgress progress) {
        if (callback != null) {
