...
public class LibraryView extends ScrollPane {
    ...
    public LibraryView() {
        ...
        subtitleLabel = new Label(
            libraryService.getInstalledCount() + " apps installed"
        );
        ...
        refreshBtn.setStyle(
            "-fx-background-color: #27272a; -fx-text-fill: white; -fx-background-radius: 6px; -fx-cursor: hand;"
        );
    ...
    private VBox buildInstalledList() {
        ...
        if (apps.isEmpty()) {
            Label emptyLabel = new Label(
                "No apps installed yet. Install apps from the Discover tab."
            );
    ...
    private HBox createLibraryRow(InstalledApp app) {
        ...
        row.setStyle(
            "-fx-background-color: #18181b; -fx-background-radius: 8px; -fx-border-color: #27272a; -fx-border-radius: 8px;"
        );
        ...
        ((Label) metaRow.getChildren().get(1)).setStyle(
            "-fx-text-fill: #71717a;"
        );
        ...
        updateBtn.setStyle(
            "-fx-background-color: #6366f1; -fx-text-fill: white; -fx-font-size: 11px; -fx-padding: 6 12; -fx-background-radius: 6px; -fx-cursor: hand;"
        );
        ...
        updateBtn.setOnAction(e -> {
            ...
            Timeline timeline = new Timeline(
                new KeyFrame(Duration.seconds(1), ev -> {
                    ...
                    updateBtn.setStyle(
                        "-fx-background-color: #22c55e; -fx-text-fill: white; -fx-font-size: 11px; -fx-padding: 6 12; -fx-background-radius: 6px;"
                    );
        ...
        openBtn.setStyle(
            "-fx-background-color: #27272a; -fx-text-fill: white; -fx-font-size: 11px; -fx-padding: 6 12; -fx-background-radius: 6px; -fx-cursor: hand;"
        );

        Button removeBtn = new Button("Remove");
        removeBtn.setStyle(
            "-fx-background-color: transparent; -fx-text-fill: #ef4444; -fx-font-size: 11px; -fx-padding: 6 12; -fx-background-radius: 6px; -fx-border-color: #ef4444; -fx-border-radius: 6px; -fx-cursor: hand;"
        );
    ...
    private void switchTab(String tabName) {
        ...
        tabButtons.forEach((name, btn) -> {
            if (name.equals(tabName)) {
                btn.setStyle(
                    "-fx-background-color: #27272a; -fx-text-fill: white; -fx-background-radius: 6px; -fx-font-weight: bold; -fx-cursor: hand; -fx-padding: 8 16;"
                );
            } else {
                btn.setStyle(
                    "-fx-background-color: transparent; -fx-text-fill: #a1a1aa; -fx-background-radius: 6px; -fx-font-weight: bold; -fx-cursor: hand; -fx-padding: 8 16;"
                );
            }
        });
